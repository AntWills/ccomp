...
public class EventBlazeTest {
    ...
    @Test
    void shouldPaginateWhereUserIsEditorUsingBlaze() {
        // Setup: Inserir 3 eventos com datas crescentes
        Event event1 = persistEventWithEditor("Evento 1", baseTime);
        Event event2 = persistEventWithEditor("Evento 2", baseTime.plusDays(1));
        Event event3 = persistEventWithEditor("Evento 3", baseTime.plusDays(2));

        // Teste: Busca inicial (Página 1 - limit 2)
        List<EventListItemView> page1 = eventBlaze.findAllWhereUserIsEditor(editorId, null, 2);

        assertThat(page1).hasSize(2);
        assertThat(page1.get(0).getId()).isEqualTo(event3.getId()); // Mais recente
        assertThat(page1.get(1).getId()).isEqualTo(event2.getId());
        ...
        List<EventListItemView> page2 = eventBlaze.findAllWhereUserIsEditor(editorId, cursor, 2);

        assertThat(page2).hasSize(1);
        assertThat(page2.get(0).getId()).isEqualTo(event1.getId());
    }
    ...
    void shouldPaginateWhereUserIsEditorUsingQueryDsl() {
        ...
        // Teste: Busca inicial (Página 1 - limit 2)
        List<Event> page1 = eventBlaze.findAllWhereUserIsEditorDsl(editorId, null, 2);
        ...
        // Teste: Próxima página (Página 2 usando o cursor do último item)
        EventCursor cursor = new EventCursor(event2.getStartDate(), event2.getId());
        List<Event> page2 = eventBlaze.findAllWhereUserIsEditorDsl(editorId, cursor, 2);

        assertThat(page2).hasSize(1);
        assertThat(page2.getFirst().getId()).isEqualTo(event1.getId());
    }
